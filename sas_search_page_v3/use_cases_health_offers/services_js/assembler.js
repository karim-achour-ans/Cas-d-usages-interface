 ...
 *
 * Looks in Organization.type[].coding[].code (ANS ValueSet).
 * Normalises to lowercase for consistency with SAS_LABEL_MAP in renderer.js.
 *
...
export async function buildOffersFromBundle(bundleUrl) {
  ...
  return roles.map(practitionerRole => {
    ...
    const practitioner = resolveReference(
      practitionerRole.practitioner?.reference, index
    );

    const slots = slotsByRoleId.get(practitionerRole.id) ?? [];

    const healthcareService = resolveReference(
      practitionerRole.healthcareService?.[0]?.reference, index
    );

    // ── Organisation (CPTS / MSP) ─────────────────────────────────────────
    // PractitionerRole.organization est la référence à la structure d'appartenance.
    // Absente pour les PS libéraux sans rattachement CPTS/MSP → null sans warning.
    const organization = practitionerRole.organization?.reference
      ? resolveReference(practitionerRole.organization.reference, index)
      : null;
    ...
    const offer = parseOffer({
      ...
      slots,
      healthcareService,
      organization,   // transmis à parser.js pour extraction future si besoin
    });

    // ── Champs org* : garantie d'alimentation même si parser.js ne les gère pas encore ──
    // Pattern "default coalescing" : on ne surécrit pas si parser.js les a déjà remplis.
    return {
      ...offer,
      orgId:   offer.orgId   ?? organization?.id   ?? null,
      orgName: offer.orgName ?? organization?.name ?? null,
      orgType: offer.orgType ?? extractOrgType(organization),
    };
  });
}