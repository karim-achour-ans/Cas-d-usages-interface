...
function getFilters() {
  return {
    ...
    profession: getChecked(['filter-medecin', 'filter-infirmier', 'filter-kine']),
...
function applyFilters() {
  ...
  cards.forEach(card => {
    ...
    // ── PROFESSION filter ────────────────────────────────────────────────
    if (visible && filters.profession.length) {
      visible = filters.profession.includes(card.dataset.profession);
    }
    ...
    if (visible && filters.mode.length) {
      visible = filters.mode.includes(card.dataset.mode);
    }
...
function setupFilters() {
  ...
  initMultiSelectDropdown('profession-filter-btn', 'profession-filter-panel');
...
function updateFilterCounts() {
  const counts = {
    ...
    profession: getChecked(['filter-medecin', 'filter-infirmier', 'filter-kine']).length,
